...
import { Link, MapPin, Check, ChevronRight, X, Globe, Layout, Target } from 'lucide-react';
...
export const CollectionAssigner = ({ collectionName, onSave, onCancel }: CollectionAssigner) => {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'category': return <Layout size={18} />;
      case 'page': return <Globe size={18} />;
      case 'section': return <Target size={18} />;
  ...
  return (
    <div className="bg-white border-8 border-black p-8 shadow-[15px_15px_0px_0px_rgba(255,215,0,1)] max-w-4xl mx-auto">
      ...
      <div className="flex justify-between items-start border-b-4 border-black pb-6 mb-8">
        <div>
          <h2 style={comicSansBold} className="text-3xl uppercase italic leading-none">Assign Collection</h2>
        ...
        <button onClick={onCancel} className="p-2 border-2 border-black hover:bg-gray-100 transition-colors">
          <X size={20} />
        </button>
      </div>
      ...
      <div className="space-y-4 mb-10">
        <label style={comicSansBold} className="block uppercase text-xs tracking-widest flex items-center gap-2 mb-4">
          <MapPin size={16} className="text-[#FFD700]" /> Select Deployment Locations
        </label>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`p-2 border-2 ${isSelected ? 'border-[#FFD700]' : 'border-black'}`}>
                    {getIcon(target.type)}
                  </div>
                  <div className="text-left">
                    <p className="font-black uppercase text-[11px] tracking-tight">{target.label}</p>
                    <p className={`text-[9px] uppercase font-bold ${isSelected ? 'text-[#FFD700]/50' : 'text-gray-400'}`}>
                      Type: {target.type}
                    </p>
                  </div>
                </div>
                {isSelected ? <Check size={20} /> : <ChevronRight size={20} className="text-gray-200 group-hover:text-black" />}
              </button>
            );
          })}
        </div>
      ...
      <div className="flex items-center justify-between gap-6 pt-6 border-t-2 border-gray-100">
        <div className="text-left">
          <p className="font-black uppercase text-[10px] text-gray-400">Deploying to:</p>
          <p className="font-black text-lg">{selectedTargets.length} Locations</p>
        </div>
        <button 
